    ...
    <div className="relative flex h-screen w-screen flex-col overflow-hidden bg-[#020611] text-white">
      ...
      {activeTab === 'HOME' ? (
        <main className="relative z-10 flex flex-1 flex-col justify-between overflow-hidden">
          ...
          >
            <ProjectPreviews onNavigate={setActiveTab} />
          </div>
          ...
          <div className={`absolute bottom-3 left-1/2 z-20 flex -translate-x-1/2 items-center gap-5 rounded border border-cyan-400/25 bg-[#020a12]/70 px-5 py-1.5 font-mono text-[9px] tracking-[0.12em] text-cyan-200/75 backdrop-blur-sm transition-opacity ${isFocusMode ? 'pointer-events-none opacity-0' : 'opacity-100'}`}>
            <span>φ = 1.6180339887</span><span>π = 3.1415926536</span><span>Fₙ = Fₙ₋₁ + Fₙ₋₂</span><span>GIE GRID: ONLINE</span>
          </div>
        </main>
      ) : (
        /* OTHER SITE PAGES */
