...
export default function App() {
  ...
  const [viewMode, setViewMode] =
    useState<ViewMode>('ORBIT_VIEW');

  const [selectedNode, setSelectedNode] =
    useState<GeoNode | null>(null);
  ...
  // Modals
  const [isLaunchEngineOpen, setIsLaunchEngineOpen] =
    useState(false);

  const [
    isBlueprintLibraryOpen,
    setIsBlueprintLibraryOpen
  ] = useState(false);

  const [
    isMathVisualizerOpen,
    setIsMathVisualizerOpen
  ] = useState(false);
  ...
  return (
    <div className="relative flex h-screen w-screen flex-col overflow-hidden bg-[#020611] text-white">
      ...
      {/* FOCUS MODE BANNER */}
      {isFocusMode && (
        <div className="absolute left-1/2 top-3 z-50 flex -translate-x-1/2 items-center space-x-3 rounded-full border border-amber-400/80 bg-amber-950/90 px-4 py-1.5 text-[10px] font-bold uppercase tracking-wider text-amber-200 shadow-[0_0_20px_rgba(255,183,0,0.4)] backdrop-blur-md">
          <span>
      ...
      {activeTab === 'HOME' ? (
        <main className="relative z-10 min-h-0 flex-1 overflow-hidden">
          ...
          {/* UNDER CONSTRUCTION */}
          <div className="pointer-events-none absolute left-1/2 top-1 z-30 -translate-x-1/2 rounded border border-amber-400/45 bg-[#07101a]/70 px-3 py-1 font-mono text-[9px] font-bold tracking-[0.28em] text-amber-300 shadow-[0_0_12px_rgba(255,183,0,.18)] backdrop-blur-sm">
            SITE IS UNDER CONSTRUCTION
          </div>

          {/* LIVE 3D GLOBE — GLOBE SIZE UNCHANGED */}
          ...
          {/* LEFT + RIGHT TOOLBARS */}
          <div
            className={`pointer-events-none absolute inset-0 z-20 flex items-start justify-between px-2 pt-2 transition-opacity duration-500 ${
              isFocusMode
                ? 'opacity-0'
                : 'opacity-100'
            }`}
          ...
          {/* BOTTOM MATHEMATICAL STATUS BAR */}
          <div
            className={`absolute bottom-0 left-0 right-0 z-30 h-[76px] border-t border-cyan-400/45 bg-[#020a12]/94 font-mono shadow-[0_-4px_20px_rgba(0,245,255,0.08)] backdrop-blur-md transition-opacity duration-500 ${
              isFocusMode
                ? 'pointer-events-none opacity-0'
                : 'opacity-100'
            }`}
          >
            <div className="grid h-full grid-cols-7 divide-x divide-cyan-400/20">
                ...
                ([
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      <div
                        className={`truncate text-[16px] font-bold leading-tight ${color}`}
                      >
                        {value}
                      </div>

                      <div
                        className={`mt-1 truncate text-[10px] font-bold tracking-[0.08em] ${color}`}
                      >
                        {label}
                      </div>
                    </div>
              ...
              <div className="flex flex-col items-center justify-center text-cyan-300">
                ...
                <div className="mt-1 text-[10px] font-bold tracking-[0.16em] text-cyan-200/75">
                  LIVE ENGINE
                </div>
              </div>
            </div>
      ...
      <button
        ...
        className={`fixed bottom-[86px] right-4 z-40 flex cursor-pointer items-center space-x-2 rounded-lg border px-5 py-3 font-mono text-sm font-bold uppercase tracking-widest backdrop-blur-md transition-all ${
      ...
      <CropCircleBlueprintModal
        isOpen={isCropBlueprintOpen}
        onClose={() =>
          setIsCropBlueprintOpen(false)
        }
      />
    </div>
  );
